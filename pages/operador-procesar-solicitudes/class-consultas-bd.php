...
class consultas_bd{
	...
	//obtengo detalle de la solicitud a procesar
	public function obtener_detalle_solicitud($id_solicitud){
		try{
			$conex= new bd_conexion();
			$conex=$conex->bd_conectarse();
			$sql="SELECT solicitudes.id_sol,
			solicitudes.desc_clie,
			solicitudes.num_ide,
			solicitudes.email,
			solicitudes.telf,			
			solicitudes.num_cuenta,
			solicitudes.monto_soli,
			solicitudes.fecha_soli,
			cate_banco.deno_banco,
			cate_tipo_cuenta.tipo_cuenta 
			 FROM (solicitudes  
			INNER JOIN cate_banco on cate_banco.id_banco=solicitudes.id_banco 
			INNER JOIN cate_tipo_cuenta ON cate_tipo_cuenta.id_cate_tipo_cuenta = solicitudes.id_cate_tipo_cuenta
			) 
			where solicitudes.id_sol=:id_sol";
			$data=array('id_sol'=>$id_solicitud);
			$consulta = $conex->prepare($sql); //evita sql injection
			$resultado = $consulta->execute($data); //se puede o no guardar en variable por si se usa luego
			//$filas = $consulta->fetchAll(\PDO::FETCH_OBJ);				
			return $consulta;
